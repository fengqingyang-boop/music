...
class MusicPlayer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.song_title_label.setStyleSheet("color: #34495e; padding: 10px; background-color: #ecf0f1; border-radius: 10px;")
    ...
    def add_files(self):
        ...
        if files:
            ...
            self.status_label.setText(f"已添加 {len(files)} 个文件，共 {len(self.playlist)} 首歌曲")
            
            if self.current_index == -1 and self.playlist:
                self.current_index = 0
                self.load_song(self.playlist[0])
    ...
    def add_folder(self):
        ...
        if folder:
            ...
            self.status_label.setText(f"已从文件夹添加 {files_added} 个文件，共 {len(self.playlist)} 首歌曲")
    ...
    def start_playback(self):
        if self.current_index >= 0 and self.current_index < len(self.playlist):
            ...
            self.is_paused = False
            self.play_button.setText("⏸ 暂停")
            self.status_label.setText(f"正在播放: {os.path.basename(self.playlist[self.current_index])}")
    ...
    def handle_error(self, error):
        ...
        QMessageBox.warning(self, "播放错误", f"播放出错: {error_msg}")
